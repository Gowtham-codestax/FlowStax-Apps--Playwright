...
import { KBZPayCenterMaker1Page } from '../../pages/KYC Verifcation App Pages/KBZPayCenterMaker1Page';
...
const { branchSr, kbzPayCenterSr, kbzPayCenterChecker1, kycOfficer, clusterMgr1 } =
  loginData.KYCVerification;

test.describe.serial('CustomerUrgentRequestFlow', () => {
  ...
  test('CustomerUrgentRequestFlowTest', async () => {
    ...
    await login.login(branchSr.username, branchSr.password);
  ...
  // priority 2 - dependsOnMethods = CustomerUrgentRequestFlowTest
  ...
  // The following methods were fully commented out in the Java source.
  // Preserved as test.skip so the flow/intent is not lost.

  test('KBZPayCenterChecker1Flow', async () => {
  ...
  test('KBZPayCenterMaker1Flow', async () => {

    const login = new LoginPage(page);
    const Kyc = new KYCPage(page);
    const KbzPaycentercheck1 = new KBZPayCenterChecker1Page(page);
    const KbzPaycenterMaker1 = new KBZPayCenterMaker1Page(page);
    const customerUgerntreq = new customerUrgentRequestPages(page);

    await login.login(kbzPayCenterSr.username, kbzPayCenterSr.password);

    await KbzPaycenterMaker1.eyeIconBtn();

    await KbzPaycenterMaker1.reSubmitBtn();
  ...
  test('ClustuerManger1PageFlow', async () => {
    

    const login = new LoginPage(page);
    const Kyc = new KYCPage(page);
    const KbzPaycentercheck1 = new KBZPayCenterChecker1Page(page);
    const ClustuerManger1 = new ClustuerManger1Page(page);

    await login.login(clusterMgr1.username, clusterMgr1.password);

    await KbzPaycentercheck1.KYCserviceReqOPtion();

    await ClustuerManger1.eyeIconBtn();

    await ClustuerManger1.sendtoKYCOfficerbtn();
