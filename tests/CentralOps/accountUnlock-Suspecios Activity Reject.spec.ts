...
import {SOCPage} from '@pages/CentralOpsPage/SOCPage';

const { branchTeam, FMU, CIFGrop , SOCCops} = loginData.CentralOps;
test.describe.serial('AccountUnlock - Suspicious Activity Detected Case - Reject', () => {
...
test.beforeAll(async ({browser})=>{
 ...
 test('FMU Login Flow Test - Escalate ', async()=>{
    ...
    await accountUnlock_Paygilant.escalatebtn();
 ...
 test('SOC Login - Reject',async()=>{

    
    const login = new LoginPage(page);
    const Kyc = new KYCPage(page)
    const SOC= new SOCPage(page);
    const Kyc1 = new KYC1_COPS_Page(page);

    await login.login(SOCCops.username, SOCCops.password);

    await SOC.PullOptionButon();
